...
# Le frein s'active si on est en forward puis cmd_speed <= 0.
# Faire Forward -> 0.0 impose un délai ! Utiliser : deadzone > cmd_speed > 0 si on veut arrêter depuis forward sans frein (ni délai)

class ESCState(Enum):
    BRAKING_REV = -2
    REVERSE = -1             # cmd_speed < 0 
    NEUTRE_REV = 0          # cmd_speed = 0
    FORWARD = 1             # cmd_speed > 0 (si cmd_speed == 0 -> brake)
...
class CommandSpeedNode(Node):
    def __init__(self):
        ...
        self.DURATION_BRAKING_1b = 0.2                     # sécurité car fourche optique pas assez précise
        ...
        self.DURATION_BRAKING_REV = 0.02
        ...
        self.braking_1b_timer = self.create_timer(self.DURATION_BRAKING_1b, self.finish_braking_1b)
        ...
        self.braking_rev_timer = self.create_timer(self.DURATION_BRAKING_REV, self.finish_braking_rev)
        self.braking_rev_timer.cancel()

        # memorisation cmd_speed si ESC occupé
        self.pending_cmd_speed = None
    ...
    def process_cmd(self, cmd_speed: float):
        self.pending_cmd_speed = None                     # nouvelles valeurs écrasent celles pending
        ...
        if self.state == ESCState.REVERSE:
            if cmd_speed <= 0:
                ...
                if self.speed_meas > self.SEUIL_FOURCHE:
                    # REVERSE -> BRAKING_REV : permet de synchroniser motor_direction
                    self.pending_cmd_speed = cmd_speed
                    self.state = ESCState.BRAKING_REV
                    self.publish_esc_state()
                    self.start_braking_rev()
                else:
                    # REVERSE -> FORWARD : en faite direct pour l'ESC (pas besoin de sécurité)
                    self.forward(cmd_speed)             # on suppose que si PWM > PWM_FORWARD_MIN alors ESC passe en Forward (indépendant de la vitesse)
                    self.state = ESCState.NEUTRE_REV
                    self.publish_esc_state()
                return
    
        elif self.state == ESCState.FORWARD:
            if cmd_speed > 0:                           # si cmd_speed == 0 : passe en BRAKING_FWD
                self.forward(cmd_speed)
                return
            else:
                # FORWARD -> REVERSE : BRAKING_FWD
                self.pending_cmd_speed = cmd_speed          # remember cmd_speed for after BRAKING_FWD (if not overwritten)
            ...
            return
            # if cmd_speed <= 0:
            #     # BRAKE -> REVERSE (cmd_speed <= 0)
            #     # wait for end of braking duration to process cmd_speed
            #     self.pending_cmd_speed = cmd_speed
            #     return
            # else: # cmd_speed > 0
            #     # BRAKING_FWD -> (REVERSE ->) FORWARD
            #     # il est dangereux de cancel braking_2 : changement de sens brutal du courant dans le circuit
            #     # On ne peut pas interrompre la sequence de freinage car on se saura plus dans quel état est l'ESC (s'il a assez freiné ou pas : fourche pas assez précise)
            #     self.pending_cmd_speed = cmd_speed
            #     return
            ...
            if cmd_speed > 0:
                ...
                self.reverse(cmd_speed)
                return
        else : #self.state == ESCState.NEUTRE_REV:
            if cmd_speed <= 0:
                if cmd_speed < 0:
                    self.state = ESCState.REVERSE
                    self.publish_esc_state()
    ...
    # FORWARD / REVERSE
    def forward(self, cmd_speed):
        if cmd_speed < self.CMD_DEADZONE:
            pwm = self.PWM_NEUTRAL
        else:
            # cmd_speed > 0
            pwm = int(self.PWM_FORWARD_MIN + cmd_speed * (self.PWM_FORWARD_MAX - self.PWM_FORWARD_MIN))
        self.publish_pwm(pwm)
        if self.debug:
            self.get_logger().info(f"FORWARD PWM: {pwm}\n\r")

    def reverse(self, cmd_speed):
        if cmd_speed > - self.CMD_DEADZONE:
            pwm = self.PWM_NEUTRAL
        else: 
            # cmd_speed < 0
            pwm = int(self.PWM_REVERSE_MIN + cmd_speed * (self.PWM_REVERSE_MIN - self.PWM_REVERSE_MAX))
        self.publish_pwm(pwm)
        if self.debug:
    ...
    def finish_braking_rev(self):
        ...
        self.state = ESCState.REVERSE
        self.publish_esc_state()
        if self.pending_cmd_speed is not None:
            cmd = self.pending_cmd_speed
    ...
    def finish_braking_2(self):
        ...
        # BRAKING -> REVERSE (car arrêt garanti par finish braking 1)
        self.state = ESCState.REVERSE
        self.publish_esc_state()
        if self.pending_cmd_speed is not None:
            cmd = self.pending_cmd_speed
